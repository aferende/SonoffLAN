    ...
    async def _process_ws_msg(self, data: dict):
        if "action" not in data:
            ...
            if data.get("error", 0) not in (0, None):
                ...
                # A ZBBridge child can apply a command while its cloud response is
                # still an error (notably 411/504). The registry reconciles those
                # replies before deciding whether this is a real failure. Keep the
                # initial response in debug logs so Home Assistant automations do
                # not notify a user about a command that actually succeeded.
                _LOGGER.debug(
